...
parser.add_argument("--opt-batch-size", type=int, help="opt batch size for TRT engine, default 1", default=20)
parser.add_argument("--max-batch-size", type=int, help="max batch size for TRT engine, default 1", default=100)
parser.add_argument("--variant", type=str, help="variant of T5, default t5-small", default="t5-base")
...
hf_model_path="/home/wzp/t5-onnx/t5-base/"
t5_model = T5ForConditionalGeneration.from_pretrained(hf_model_path) 
tokenizer = T5Tokenizer.from_pretrained(hf_model_path)
config = T5Config.from_pretrained(hf_model_path)

# save model locally
pytorch_model_dir = hf_model_path
...
# convert to ONNX
onnx_model_path = '/home/wzp/t5-onnx/onnx'
os.makedirs(onnx_model_path, exist_ok=True)

metadata=NetworkMetadata(variant=T5_VARIANT, precision=Precision(fp16=args.fp16), other=T5Metadata(kv_cache=False))

encoder_onnx_model_fpath = T5_VARIANT + "-encoder-12.onnx"
decoder_onnx_model_fpath = T5_VARIANT + "-decoder-with-lm-head-12.onnx"
